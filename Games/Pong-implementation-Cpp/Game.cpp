...
bool Game::Initialize() {
    ...
    if (SDL_Init(SDL_INIT_JOYSTICK) < 0) {
        SDL_Log("Joystick initialization failed! Error message: %s", SDL_GetError());
        return false;
    }
...
void Game::ProcessInput() {
    ...
    if (SDL_JoystickGetButton(SDL_JoystickOpen(0), 0))
        mIsRunning = false;

    // Checking for joystick state
    js_state = SDL_JoystickGetAxis(SDL_JoystickOpen(0), 1);
}
...
//    while (!SDL_TICKS_PASSED(SDL_GetTicks(), mTicksCount + 16));
    ...
    paddle1->changePaddlePositionJoy(js_state, deltaTime);
